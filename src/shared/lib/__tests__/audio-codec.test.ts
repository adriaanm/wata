...
function assertBuffersSimilar(
  ...
  if (expectedRms > 0.1 && actualMax < 1000) {
    throw new Error(
      `Decoded audio appears silent (max=${actualMax}) but expected signal`,
    );
  }
}
...
describe('encodeOggOpus', () => {
    ...
    it('should encode 16kHz Int16Array PCM to Ogg Opus', () => {
      ...
      for (let i = 0; i < samples.length; i++) {
        ...
        samples[i] = Math.round(
          Math.sin((i * 2 * Math.PI) / OPUS_FRAME_SIZE) * 16000,
        );
      }
  ...
  describe('error handling', () => {
    it('should throw on unsupported channel count', () => {
      ...
      expect(() => {
        encodeOggOpus(
          samples,
          { sampleRate: 16000, channels: 2 as 1 },
          mkEncoder,
        );
  ...
  describe('with logger', () => {
    it('should log encoding progress', () => {
      ...
      expect(logger.logs.some(l => l.includes('encodeOggOpus: starting'))).toBe(
        true,
      );
      expect(logger.logs.some(l => l.includes('encodeOggOpus: complete'))).toBe(
        true,
      );
    });
    ...
    it('should log Int16Array to Float32Array conversion', () => {
      ...
      expect(
        logger.logs.some(l =>
          l.includes('converting Int16Array to Float32Array'),
        ),
      ).toBe(true);
    });
    ...
    it('should log resampling when input rate differs', () => {
      ...
      expect(
        logger.logs.some(l => l.includes('resampling') && l.includes('44100')),
      ).toBe(true);
...
describe('decodeOggOpus', () => {
  describe('basic decoding', () => {
    ...
    it('should calculate correct duration', () => {
      ...
      const expectedDuration =
        (OPUS_FRAME_SIZE * frameCount) / OPUS_SAMPLE_RATE;
    ...
    it('should always return sampleRate of 16000', () => {
      ...
      for (const rate of testRates) {
        const samples = new Float32Array(
          Math.ceil((OPUS_FRAME_SIZE * 2 * rate) / 16000),
        );
        for (let i = 0; i < samples.length; i++) {
  ...
  describe('with logger', () => {
    it('should log decoding progress', () => {
      ...
      expect(logger.logs.some(l => l.includes('decodeOggOpus: starting'))).toBe(
        true,
      );
      expect(logger.logs.some(l => l.includes('decodeOggOpus: complete'))).toBe(
        true,
      );
    });
  });
  ...
  describe('decoded data quality', () => {
    ...
    it('should handle silence correctly', () => {
      ...
      for (
        let i = OPUS_PRE_SKIP;
        i < Math.min(result.pcm.length, samples.length);
        i++
      ) {
...
describe('Roundtrip: encode → decode', () => {
  ...
  describe('roundtrip with Int16Array input', () => {
    it('should handle Int16Array input correctly', () => {
      ...
      for (let i = 0; i < original.length; i++) {
        original[i] = Math.round(
          Math.sin((i * 2 * Math.PI * 440) / 16000) * 16000,
        );
      }
  ...
  describe('roundtrip edge cases', () => {
    ...
    it('should handle silence', () => {
      ...
      for (
        let i = OPUS_PRE_SKIP;
        i < Math.min(decoded.pcm.length, original.length);
        i++
      ) {
    ...
    it('should handle full-scale positive', () => {
      ...
      for (let i = 0; i < original.length; i++) {
        original[i] =
          Math.sin((i * 2 * Math.PI * 440) / OPUS_SAMPLE_RATE) * 1.0;
      }
  ...
  describe('complex audio patterns', () => {
    ...
    it('should handle amplitude sweep', () => {
      ...
      for (let i = 0; i < samples.length; i++) {
        ...
        samples[i] =
          Math.sin((i * 2 * Math.PI * 440) / OPUS_SAMPLE_RATE) * amplitude;
      }
      ...
      const firstQuarterEnergy =
        decoded.pcm.slice(0, firstQuarter).reduce((sum, v) => sum + v * v, 0) /
        firstQuarter;
