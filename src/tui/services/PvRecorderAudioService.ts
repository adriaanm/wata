...
/**
 * Audio service using PvRecorder + shared audio library (no FFmpeg dependency)
 *
 * Architecture:
 * - PvRecorder captures PCM audio (Int16Array frames, 16kHz, 512 samples/frame)
 * - Raw PCM samples are accumulated during recording
 * - Shared audio library encodes to Ogg Opus format
 *
 * Benefits of 16kHz:
...
export class PvRecorderAudioService {
  ...
  async stopRecording(): Promise<RecordingResult> {
    ...
    try {
      ...
      // Encode to Ogg Opus using shared audio library
  ...
  async startPlayback(audioUrl: string, accessToken?: string): Promise<void> {
    ...
    try {
      ...
      if (needsConversion) {
        ...
        const result: DecodeResult = decodeOggOpus(audioBuffer, { logger });
        ...
        for (let i = 0; i < result.pcm.length; i++) {
          pcmFloat32[i] = result.pcm[i] / 32768;
        }

        // Encode to WAV and write to file
        const wavBuffer = encodeWav(pcmFloat32, this.OPUS_SAMPLE_RATE);
        await writeFile(wavPath, wavBuffer);

        LogService.getInstance().addEntry(
          'log',
          `Decoded Ogg Opus to WAV (${result.pcm.length} samples, ${result.duration.toFixed(2)}s)`,
        );
