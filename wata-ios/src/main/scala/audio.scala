 ...
 *  package `github.com/adriaanm/wata/go-pkgs/macaudio`, the same backend
 *  wata-mac binds (plan 0063): the package was written iOS-clean and the one
 *  iOS-only piece — AVAudioSession activation — lives inside it
 *  (session_ios.go), invisible at this seam. The DECLARATIONS are identical
 *  to `wata-fb/src/main/scala/audio.scala`'s, which is what lets
 *  `audiothread.scala` be ONE file symlinked into all three apps: the seam
 *  between the backends is the `@go.bind` path on this object and nothing
 *  else. `just facade-check` enforces that identity — perturb a signature
 *  here and the gate says so, naming both files. */
