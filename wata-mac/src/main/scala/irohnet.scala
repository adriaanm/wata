 ...
 *  ordinary net/http Client facade type, so `MacHttp` carries it unchanged —
 *  nothing above the capability line knows the transport changed.
 *
 *  This declares only what the mac client calls: the away-from-home parent
 *  dials a homeserver they cannot reach over TCP (plan 0034). Enrolment is
 *  the handset's story — `EnsureKey`/`LastRefusal` are wata-fb's facade and
 *  deliberately absent here, which is why the pair `just facade-check` holds
 *  this file against is wata-tui's, not wata-fb's.
 *
 *  The real transport is compiled in only on darwin with the `iroh` Go build
 *  tag (`just mac-iroh-build` / `just mac-iroh-smoke`); every other build gets
 *  the package's loud-error stub, which is what keeps `just mac-build`
 *  cargo-free. */
