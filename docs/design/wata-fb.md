...
mostly ASCII plus a handful of custom icon glyphs at 0x80-0x8D
(battery, checkmark, wifi, signal bars, the connectivity element's
cellular mast at `Font.ICON_CELL` = 0x8C, and the favorite star at
`Font.ICON_STAR` = 0x8D) and a couple of block-element glyphs. There is
...
this module, so the cellular mast and the star were drawn by hand in
the same 5x8 style and a future regeneration has to carry both
