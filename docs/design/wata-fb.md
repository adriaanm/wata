...
mostly ASCII plus a handful of custom icon glyphs at 0x80-0x8C
(battery, checkmark, wifi, signal bars, and — the connectivity
element's — a cellular mast at `Font.ICON_CELL` = 0x8C) and a couple of
block-element glyphs. There is no vector/TrueType font path; the font
table comment says it was generated by a `scratchpad/genfont.py` script
that is not present in this module, so the cellular mast was drawn by
hand in the same 5x8 style and a future regeneration has to carry it
forward. `Font.drawText`/`drawTextCentered` lay text out on a
