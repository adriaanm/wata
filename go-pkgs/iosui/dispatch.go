// The main-thread dispatch seam: libdispatch over purego — the iOS twin of
// nativeui/dispatch.go, duplicated rather than imported because nativeui's
// module carries the AppKit-side glue (plan 0044 starts the Apple clients on
// honest duplication; fold only when both are green and the seams show).
//
// UIKit, like AppKit, is main-thread-only once a window is up, and Go's
// scheduler migrates goroutines across OS threads freely — so the apply step
// (and only the apply step: bodies and the diff stay on the frame goroutine,
// per wataui's purity rule) hops to the main queue here. dispatch_async_f is
...
// main thread runs a runloop (UIApplicationMain) — a headless test would
// wait forever on it — so tests can prove the seam on a private serial
// queue, which libdispatch drains on its own worker threads. The code path
// is identical.
...
func dispatchInit() {
	dispatchOnce.Do(func() {
		...
		if err != nil {
			panic("iosui: dlopen libSystem: " + err.Error())
		}
		...
		if err != nil {
			panic("iosui: dlsym _dispatch_main_q: " + err.Error())
		}
...
// while the main thread runs a runloop (UIApplicationMain).
