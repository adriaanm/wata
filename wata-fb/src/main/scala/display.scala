/** M8 chunk 5 — the RGB565 draw layer, ported from fbclient `display.zig`
 *  (Framebuffer) + `font.zig` (the 5x8 Spleen bitmap font, decision 5:
 *  freetype NOT ported). Device-layer app code (wata-fb).
 *
 ...
 *  fb IS 160x128x2 = 40960 bytes, direct blit — display.zig FbdevBackend) and
 *  the host PNG export reads it directly (decision 4: RGB565 everywhere, no
 *  SDL). NO class holds the buffer — the subset forbids plain non-case classes
 *  AND a `go.Bytes` field in a case class is a known landmine (wata-server's
 *  media-bytes note), so draw ops are `object` functions over a passed buffer
 *  (wata-server's object-oriented-free style).
 *
 *  Geometry is FIXED at 160x128 per display.zig; ioctl geometry discovery is
 *  NOT ported (the Zig code doesn't do it either). */
object Display:
  val W: scala.Int = 160
  val H: scala.Int = 128
  val BYTES: scala.Int = W * H * 2   // 40960 — one RGB565 frame

/** RGB565 colors — the ST7735S hardware format (display.zig `colors`).
 *  Named values are precomputed literals (rgb() is kept for dynamic use). */
object Color:
...
/** the pixel-buffer draw primitives (display.zig Framebuffer). */
object Draw:
  def newBuffer(): go.Bytes =
    // NB `go.makeSlice` maps to `make([]T,n)` only in val/statement position,
    // not in tail/return position (emitter sharp edge — see DOGFOOD).
  ...
  /** 1px border rectangle (not filled) — display.zig strokeRect. */
...
/** the 5x8 bitmap font renderer (font.zig). `bg` is drawn only when `hasBg`
 *  (the subset has no `?Color`; the has-flag idiom, like the domain layer). */
object Font:
  ...
  // custom glyph codes (font.zig icon.*)
   ...
   *  glyphs 0x80-0x8B and block elements) — flat [ch*8 + row], MSB-first
   *  (bit7 = leftmost). GENERATED from fbclient font.zig; regenerate with
   *  scratchpad/genfont.py. A constant `Array` literal (emits []int{...}),
   *  no init-time computation. decision 5: bitmap only, no freetype. */
  // M9 ch.4a (CONC-10 migration): the font table is pure bulk data —
  // `IArray` (DATA-12), crossable with zero conventions (was Array, whose
  // mutation surface made a module val unshareable).
  ...
  /** draw one glyph `code` (0..255) at pixel (px0, py0) — font.zig drawChar. */
   ...
   *  the 1px status line) — font.zig drawText. Uses the byte view (ASCII). */
