 ...
 *  view algebra's) into what UIKit wants — wata-mac's pixels.scala verbatim
 *  (honest duplication per plan 0044). Pure functions, no facade calls, so
 *  the conversions are covered by the pure arm of `wata-ios interptest`.
 *
...
object IosPixels:
   ...
   *  reads UIColor colorWithRed:green:blue:alpha: takes. */
   ...
   *  UIImageView is pre-scaled here so no interpolating scaler ever
   *  touches it. */
